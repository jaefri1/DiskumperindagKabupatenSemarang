...
Skema (6 tabel dasar + 1 tabel output):
  ...
  penduduk        → penduduk per kecamatan x tahun (2022-2025), sumber: Dukcapil
                    Kab. Semarang — dipakai untuk rasio UMKM per 1.000 penduduk
...
def buat_skema(conn: sqlite3.Connection) -> None:
    ...
    -- Penduduk per kecamatan, per tahun (sumber: Dukcapil Kab. Semarang,
    -- 2022-2025) — dipakai untuk hitung rasio UMKM per 1.000 penduduk
    ...
    """)
    conn.commit()
    print("  Skema dibuat: 8 tabel (kecamatan, sektor_umkm, koperasi, "
          "industri_kecil, aset_koperasi, penduduk, hasil_klaster, profil_klaster)")
...
def isi_penduduk(conn: sqlite3.Connection, kec_map: dict) -> None:
    """
    Sumber: Dukcapil Kab. Semarang, bukan dari data/clean/ seperti tabel
    lain — file xlsx aslinya (format khas BPS: 4 baris header, lalu 19
    baris kecamatan, ditutup 1 baris total "Kabupaten Semarang" yang
    SENGAJA tidak dimasukkan karena bisa dihitung ulang dari SUM()).
    """
    wb = load_workbook(
        DATA_DIR / "raw" / "Jumlah_Penduduk_Menurut_Kecamatan_dan_Jenis_Kelamin__2022-2025.xlsx",
        read_only=True, data_only=True,
    )
    rows = list(wb["Sheet1"].iter_rows(values_only=True))
    data_rows = rows[4:23]          # baris 4-22 = 19 kecamatan
    tahun_list = [2022, 2023, 2024, 2025]

    records, tidak_ketemu = [], []
    for row in data_rows:
        if row[0] not in kec_map:
            tidak_ketemu.append(row[0])
            continue
        kid = kec_map[row[0]]
        for i, tahun in enumerate(tahun_list):
            base = 1 + i * 3   # blok 3 kolom per tahun: Laki-laki, Perempuan, Total
            records.append((kid, tahun, int(row[base]), int(row[base + 1]), int(row[base + 2])))

    if tidak_ketemu:
        print(f"  ⚠ Nama kecamatan tidak cocok dengan tabel kecamatan: {tidak_ketemu}")

    conn.executemany(
        "INSERT INTO penduduk (kecamatan_id, tahun, laki_laki, perempuan, jumlah_penduduk) "
        "VALUES (?,?,?,?,?)", records,
    )
    conn.commit()
    print(f"  Tabel penduduk: {len(records):,} baris (19 kecamatan x {len(tahun_list)} tahun)")
...
def verifikasi(conn: sqlite3.Connection) -> None:
    ...
    tabel_list = ["kecamatan", "sektor_umkm", "koperasi",
                   "industri_kecil", "aset_koperasi", "penduduk",
                   "hasil_klaster", "profil_klaster"]
    for t in tabel_list:
